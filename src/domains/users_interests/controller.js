...
const removeInterest = async (interestId, userId) => {
  try {
    ...
    const userInterestIndex = user.interest.findIndex(
      (e) => e._id.toString() === interestId
    );
...
const addInterest = async (interestId, userId) => {
  try {
    ...
    const user = await User.findOne({ _id: userId });
    const userInterests = user.interest.find(
      (e) => e._id.toString() === interestId
    );
    if (userInterests) {
...
const updateInterest = async (interestId, data) => {
  try {
    // updated interest
    const updatedInterest = await UsersInterest.updateOne(
      { _id: interestId },
      data
    );
