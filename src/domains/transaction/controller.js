...
const createPendingTransaction = async (senderId,receiverId,amount,senderWalletTransactionId,receiverWalletTransactionId) => {
    try{
...
const checkTransactionStatus = async (transaction) => {
 // return message
 if(transaction.isApproveByReceiver === true && transaction.isApproveBySender === null){
    const message = "Transaction Successfully Approved By Receiver"
    return message;
 }
 if(transaction.isApproveByReceiver === null && transaction.isApproveBySender === true){
    const message = "Transaction Successfully Approved By Sender"
    return message;
 }
 if(transaction.isApproveByReceiver === false && transaction.isApproveBySender === null){
    const message = "Transaction Successfully disApproved By Receiver"
    return message;
 }
 if(transaction.isApproveByReceiver === null && transaction.isApproveBySender === false){
    const message = "Transaction Successfully disApproved By Sender"
    return message;
 }

    // make update
    if(transaction.isApproveByReceiver === true && transaction.isApproveBySender === true){
        await updateWallet(transaction.receiverId, transaction.amount);
        await updateWalletTransaction(transaction.senderWalletTransactionId, 'successful');
        await updateWalletTransaction(transaction.receiverWalletTransactionId, 'successful');
        await deleteUserPendingTransaction(transaction._id);
        const message = "Transaction Successfully Completed"
        return message;
    }
    if(transaction.isApproveByReceiver === false && transaction.isApproveBySender === true){
        await updateWallet(transaction.senderId, transaction.amount);
        await updateWalletTransaction(transaction.senderWalletTransactionId, 'failed');
        await updateWalletTransaction(transaction.receiverWalletTransactionId, 'failed');
        await deleteUserPendingTransaction(transaction._id);
        const message = "Transaction Failed Due to The Receiver Disapproved The Transaction And The Money Will Be Return Back To the Sender";
        return message;
    } 
    if(transaction.isApproveByReceiver === true && transaction.isApproveBySender === false){
        await updateWalletTransaction(transaction.senderWalletTransactionId, 'withhold');
        await updateWalletTransaction(transaction.receiverWalletTransactionId, 'withhold');
        await deleteUserPendingTransaction(transaction._id);
        const message = "Transaction Witheld Due to The Receiver Approved The Transaction And Sender Disapprove The Transaction The Money Will Be Withold";
        return message;
    } 
} 
