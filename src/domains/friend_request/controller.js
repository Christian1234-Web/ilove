...
const findUserSendRquest = async (userId) => {
    try{
        const request = await FriendRquest.find({senderId:userId}).populate("receiverId");
        return {request}
    }catch(err){
        throw err;
    }
...
const findUserRecieveRquest = async (userId) => {
    try{
        const request = await FriendRquest.find({receiverId:userId}).populate("senderId");
        return {request}
    }catch(err){
        throw err;
    }
...
module.exports = {findUserSendRquest, findUserRecieveRquest, sendRequest,deleteAllRequest,acceptRequest,rejectRequest};
